...
const sorteioSchema = z.object({
  ...
  total_perguntas: z.number().int().min(0).default(0),
})
...
export async function POST(req: NextRequest): Promise<NextResponse> {
  try {
    ...
    const { sessao_id, acertos, total_perguntas } = parsed.data
    const supabase = createServiceClient()

    // Calcula percentual de acerto (0-100)
    const percentual = total_perguntas > 0
      ? Math.round((acertos / total_perguntas) * 100)
      : 0

    // Busca todos os prêmios disponíveis ordenados do maior para o menor percentual
    ...
    // Seleciona o MELHOR prêmio que o cliente merece (maior threshold ≤ score)
    const elegivel = premiosDisponiveis.find((p) => p.percentual_acerto <= percentual)
    // Fallback: menor threshold disponível (consolação)
    const premio = elegivel ?? premiosDisponiveis[premiosDisponiveis.length - 1]

    // Registra a sessão com score e prêmio ganho
    await supabase
      .from('sessoes')
      .upsert({
        id: sessao_id,
